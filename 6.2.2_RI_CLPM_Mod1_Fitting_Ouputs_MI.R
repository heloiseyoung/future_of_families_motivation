...
if (dir.exists("/Users/pierre.jacquet/Library/CloudStorage/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Tables")) {
  table_dir <- "/Users/pierre.jacquet/Library/CloudStorage/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Tables"
} else if (dir.exists("C:/Users/PJacquet/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Tables")) {
  table_dir <- "C:/Users/PJacquet/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Tables"
} else {
  stop("Results_Tables directory not found.")
}

if (dir.exists("/Users/pierre.jacquet/Library/CloudStorage/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Objects")) {
  fit_dir <- "/Users/pierre.jacquet/Library/CloudStorage/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Objects"
} else if (dir.exists("C:/Users/PJacquet/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Objects")) {
  fit_dir <- "C:/Users/PJacquet/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Objects"
} else {
  fit_dir <- file.path(dirname(table_dir), "Fit_Objects")
  dir.create(fit_dir, recursive = TRUE, showWarnings = FALSE)
}

if (dir.exists("/Users/pierre.jacquet/Library/CloudStorage/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Figures")) {
  fig_dir <- "/Users/pierre.jacquet/Library/CloudStorage/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Figures"
} else if (dir.exists("C:/Users/PJacquet/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Figures")) {
  fig_dir <- "C:/Users/PJacquet/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_1/Model Fit_Figures"
} else {
...
n_imp <- 20
model_id <- "model_1"
output_prefix <- "model_1_RI_CLPM"

loaded_fit_objects <- ls(envir = .GlobalEnv, pattern = "^model_1_fit_imp_[0-9]+$")
...
fit_ref_file <- file.path(fit_dir, paste0(model_id, "_fit_imp_1.rds"))
...
make_target_table <- function(fit, family_name) {
  ...
  if (family_name == "bet_sign") {
    ...
    target <- pt %>%
      dplyr::filter(op == "~", free > 0)
    
  } else if (family_name == "Nu_free") {
    
    target <- pt %>%
      dplyr::filter(op == "~1", free > 0)
  ...
  target <- target %>%
    dplyr::select(any_of(c("id", "group", "lhs", "op", "rhs", "label", "free", "mat"))) %>%
    dplyr::distinct()
  
  pe_keep <- pe_raw %>%
    dplyr::select(any_of(c("group", "lhs", "op", "rhs", "est", "ci.lower", "ci.upper")))
  ...
  if (any(is.na(target$est))) {
    ...
    print(target %>% dplyr::filter(is.na(est)))
...
make_empirical_family_map <- function(fit, imp_id, draws_i, family_name) {
  ...
  if (max(mapped$abs_diff_mean, na.rm = TRUE) > 1e-6) {
    ...
    print(
      mapped %>%
        dplyr::arrange(desc(abs_diff_mean)) %>%
        dplyr::select(
...
for (i in seq_len(n_imp)) {
  ...
  diag_i <- data.frame(
    ...
    dplyr::filter(grepl(stan_regex, stan_name))
...
all_draws <- dplyr::bind_rows(draws_list)
all_diag <- dplyr::bind_rows(diag_list)
param_map_emp_all <- dplyr::bind_rows(param_map_emp_list)
...
mapping_checks <- param_map_emp_all %>%
  dplyr::group_by(imputation, family) %>%
  dplyr::summarise(
    n = n(),
...
diag_summary <- diag_mapped %>%
  dplyr::group_by(family, family_label, group, lhs, op, rhs, parameter) %>%
  dplyr::summarise(
...
summary_table <- all_draws_mapped %>%
  dplyr::filter(!is.na(lhs)) %>%
  dplyr::group_by(family, family_label, group, lhs, op, rhs, label, mat, parameter) %>%
  dplyr::summarise(
  ...
  dplyr::arrange(family_label, group, lhs, op, rhs)
...
results_beta <- summary_table %>%
  dplyr::filter(family_label == "beta") %>%
  dplyr::arrange(group, lhs, rhs)

results_theta <- summary_table %>%
  dplyr::filter(family_label == "theta") %>%
  dplyr::arrange(group, lhs, rhs)

results_psi <- summary_table %>%
  dplyr::filter(family_label == "psi") %>%
  dplyr::arrange(group, lhs, rhs)

results_nu <- summary_table %>%
  dplyr::filter(family_label == "nu") %>%
...
print(summary_table %>% dplyr::count(family_label, group))
...
sample_draws_for_plot <- function(dat, prop = figure_draw_prop) {
  ...
  dat %>%
    dplyr::group_by(stan_name, imputation) %>%
    dplyr::group_modify(~ {
...
results_beta_plot <- results_beta %>%
  dplyr::mutate(
    ...
    phase = "dynamic",
    effect_class = dplyr::case_when(
      q2.5 > 0 ~ "positive",
      q97.5 < 0 ~ "negative",
      TRUE ~ "includes zero"
    ),
    rope_class = dplyr::case_when(
  ...
  dplyr::filter(class %in% class_levels) %>%
  dplyr::distinct(group, lhs, rhs, .keep_all = TRUE)
...
for (cl in class_levels) {
  ...
  meta_cl <- results_beta_plot %>%
    ...
    dplyr::distinct() %>%
    dplyr::mutate(
      plot_key = paste(group, lhs, rhs, sep = "___")
    )
  ...
  keys_cl <- unique(meta_cl$plot_key)
  ...
  dat_cl <- all_draws_mapped %>%
    ...
    dplyr::filter(plot_key %in% keys_cl) %>%
    dplyr::select(
      stan_name,
      imputation,
      value,
  ...
  if (dplyr::n_distinct(dat_cl$value) < 2) {
  ...
  rm(meta_cl, keys_cl, dat_cl, p_density, file_stub)
  gc()
}
...
make_nonreg_forest_plot <- function(dat, title, filename, width = 10, height = 7) {
  ...
  dat <- dat %>%
    dplyr::filter(is.finite(mean), is.finite(q2.5), is.finite(q97.5)) %>%
    dplyr::arrange(group_label, pretty_param) %>%
    dplyr::mutate(
...
for (sex_i in group_levels) {
  ...
  plots_nu_forest[[sex_i]] <- make_nonreg_forest_plot(
    dat = nu_plot %>% dplyr::filter(group_label == sex_i),
  ...
  plots_theta_forest[[sex_i]] <- make_nonreg_forest_plot(
    dat = theta_plot %>% dplyr::filter(group_label == sex_i),
  ...
  plots_psi_var_forest[[sex_i]] <- make_nonreg_forest_plot(
    dat = psi_var_plot %>% dplyr::filter(group_label == sex_i),
  ...
  plots_psi_cov_forest[[sex_i]] <- make_nonreg_forest_plot(
    dat = psi_cov_plot %>% dplyr::filter(group_label == sex_i),
...
for (i in seq_len(nrow(plot_specs_other))) {
  ...
  for (sex_i in group_levels) {
    ...
    dat_i <- all_draws_mapped %>%
      ...
      dplyr::filter(!is.na(pretty_param))
    ...
    if (dplyr::n_distinct(dat_i$facet_label) == 0 || dplyr::n_distinct(dat_i$value) < 2) {
...
plot_robust <- results_beta_plot %>%
  ...
  dplyr::count(class, group_label, robustness) %>%
  dplyr::group_by(class, group_label) %>%
  dplyr::mutate(prop = n / sum(n)) %>%
  dplyr::ungroup()

plot_robust_boys <- plot_robust %>%
  dplyr::filter(group_label == "boys")
...
plot_robust_girls <- plot_robust %>%
  dplyr::filter(group_label == "girls")
...
heatmap_data <- results_beta_plot %>%
  dplyr::mutate(
    x_axis = as.character(lag)
  ) %>%
  dplyr::group_by(class, x_axis, group_label) %>%
  dplyr::summarise(
...
p_heatmap <- ggplot(heatmap_data, aes(x = x_axis, y = class, fill = mean_effect)) +
  ...
  labs(
    title = "Mean effects across developmental paths",
    subtitle = "Average posterior means by lag and sex",
...
print(p_heatmap)
save_plot(p_heatmap, paste0(model_id, "_heatmap_effects_mi"), 12, 6.5)
...
trajectory_ar <- results_beta_plot %>%
  dplyr::filter(class %in% c("ADHD AR", "EXT AR", "INT AR"), !is.na(lag))
...
trajectory_forward <- results_beta_plot %>%
  dplyr::filter(class %in% c("adhd -> ext", "adhd -> int"), !is.na(lag))
...
trajectory_backward <- results_beta_plot %>%
  dplyr::filter(class %in% c("ext -> adhd", "int -> adhd"), !is.na(lag))
...
trajectory_lateral <- results_beta_plot %>%
  dplyr::filter(class %in% c("ext -> int", "int -> ext"), !is.na(lag))
...
print(list.files(getwd(), pattern = paste0("^", model_id, "_.*\\.(png|pdf)$")))

expected_files <- list.files(
  fig_dir,
  pattern = paste0("^", model_id, "_.*\\.(png|pdf)$"),
  full.names = FALSE
)
...
cat("\nNumber of ", model_id, " figure files in fig_dir: ", length(expected_files), "\n", sep = "")
cat("Number of ", model_id, " figure files in working directory: ", length(wd_files), "\n", sep = "")
