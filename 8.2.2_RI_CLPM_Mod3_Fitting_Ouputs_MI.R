...
if (dir.exists("/Users/pierre.jacquet/Library/CloudStorage/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_3/Model Fit_Figures")) {
  fig_dir <- "/Users/pierre.jacquet/Library/CloudStorage/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_3/Model Fit_Figures"
} else if (dir.exists("C:/Users/PJacquet/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_3/Model Fit_Figures")) {
  fig_dir <- "C:/Users/PJacquet/Dropbox/DGAEFS/AXIS 0_COLLABORATORS/Heloise Young/Child Development/Short-term mindset/RI-CLPM/Model_3/Model Fit_Figures"
} else {
...
# Nettoyage de sécurité : si des objets lourds ont été chargés par erreur,
# on les enlève de l'environnement global.
loaded_fit_objects <- ls(envir = .GlobalEnv, pattern = "^model_3_fit_imp_[0-9]+$")
...
fit_ref_file <- file.path(fit_dir, "model_3_fit_imp_1.rds")
...
saveRDS(
  param_map_emp_all,
  file.path(fit_dir, "model_3_empirical_mapping_all_imputations.rds"),
  compress = "xz"
)

############################################################
############ 7. Map raw draws to readable parameters #######
############################################################
...
############################################################
######## 9. Posterior summary from raw mapped draws ########
############################################################
...
results_outcome_paths <- results_beta %>%
  filter(lhs %in% c("mating.behaviour.z", "risky.health.behaviour.z"))
...
cat("Rows in results_outcome_paths :", nrow(results_outcome_paths), "\n")
...
write.csv(
  param_map_emp_all,
  file.path(table_dir, "model_3_RI_CLPM_empirical_mapping_all_imputations.csv"),
  row.names = FALSE
)

write.csv(
  mapping_checks,
  file.path(table_dir, "model_3_RI_CLPM_empirical_mapping_checks.csv"),
  row.names = FALSE
)

write.csv(
  summary_table,
  file.path(table_dir, "model_3_RI_CLPM_posterior_summary_all_parameters_readable.csv"),
  row.names = FALSE
)

write.csv(
  results_beta,
  file.path(table_dir, "model_3_RI_CLPM_regression_results.csv"),
  row.names = FALSE
)

write.csv(
  results_theta,
  file.path(table_dir, "model_3_RI_CLPM_theta_results.csv"),
  row.names = FALSE
)

write.csv(
  results_psi,
  file.path(table_dir, "model_3_RI_CLPM_var_covar_results.csv"),
  row.names = FALSE
)

write.csv(
  results_nu,
  file.path(table_dir, "model_3_RI_CLPM_intercepts_results.csv"),
  row.names = FALSE
)

write.csv(
  results_ar,
  file.path(table_dir, "model_3_RI_CLPM_AR_results.csv"),
  row.names = FALSE
)

write.csv(
  results_cl,
  file.path(table_dir, "model_3_RI_CLPM_CL_results.csv"),
  row.names = FALSE
)

write.csv(
  results_adversity_paths,
  file.path(table_dir, "model_3_RI_CLPM_adversity_paths_results.csv"),
  row.names = FALSE
)

write.csv(
  results_outcome_paths,
  file.path(table_dir, "model_3_RI_CLPM_outcome_paths_results.csv"),
  row.names = FALSE
)

write.csv(
  all_draws,
  file.path(table_dir, "model_3_RI_CLPM_posterior_draws_all_parameters.csv"),
  row.names = FALSE
)

write.csv(
  all_draws_mapped,
  file.path(table_dir, "model_3_RI_CLPM_posterior_draws_all_parameters_mapped.csv"),
  row.names = FALSE
)

write.csv(
  all_diag,
  file.path(table_dir, "model_3_RI_CLPM_diagnostics_all_parameters_by_imputation.csv"),
  row.names = FALSE
)

write.csv(
  diag_summary,
  file.path(table_dir, "model_3_RI_CLPM_diagnostics_summary.csv"),
  row.names = FALSE
)
...
cat("\nHead of results_outcome_paths:\n")
print(head(results_outcome_paths, 20))
...
results_outcome_paths <- clean_results_df(results_outcome_paths)
...
phase_levels <- c("dynamic", "3y baseline", "5y residual", "15y outcome")
...
file_stub_fun <- function(cl) {
  case_when(
    ...
    cl == "adhd -> mating" ~ "adhd_to_mating",
    cl == "ext -> mating" ~ "ext_to_mating",
    cl == "int -> mating" ~ "int_to_mating",
    cl == "adhd -> risky health" ~ "adhd_to_risky_health",
    cl == "ext -> risky health" ~ "ext_to_risky_health",
    cl == "int -> risky health" ~ "int_to_risky_health",
    TRUE ~ "other"
  )
}
...
results_beta_plot <- results_beta %>%
  mutate(
    ...
    class = case_when(
      ...
      lhs == "mating.behaviour.z" & rhs == "wAdhd_15" ~ "adhd -> mating",
      lhs == "mating.behaviour.z" & rhs == "wExt_15"  ~ "ext -> mating",
      lhs == "mating.behaviour.z" & rhs == "wInt_15"  ~ "int -> mating",
      lhs == "risky.health.behaviour.z" & rhs == "wAdhd_15" ~ "adhd -> risky health",
      lhs == "risky.health.behaviour.z" & rhs == "wExt_15"  ~ "ext -> risky health",
      lhs == "risky.health.behaviour.z" & rhs == "wInt_15"  ~ "int -> risky health",
      TRUE ~ "other"
    ),
    lag = case_when(
    ...
    phase = case_when(
      lhs %in% c("wAdhd_3","wExt_3","wInt_3") &
        rhs %in% c("early.stochasticity.3y.z","early.volatility.3y.z","early.threat.3y.z","early.deprivation.3y.z") ~ "3y baseline",
      lhs %in% c("wAdhd_5","wExt_5","wInt_5") &
        rhs %in% c("stoch5_resid","vol5_resid","threat5_resid","depr5_resid") ~ "5y residual",
      lhs %in% c("mating.behaviour.z","risky.health.behaviour.z") ~ "15y outcome",
      TRUE ~ "dynamic"
    ),
...
############################################################
########### 16. Density plots by parameter class ###########
########### memory-safe version: one class at a time #######
############################################################

# Remove previous large beta plotting object if it exists
...
for (cl in class_levels) {
  ...
  # Downsample before plotting to reduce memory use
  dat_cl <- dat_cl %>%
  ...
  save_plot(
    p_density,
    paste0("model_3_density_", file_stub, "_mi"),
    12,
    8
  )
...
############################################################
############ 17. Forest plots by parameter class ###########
############################################################
...
for (cl in class_levels) {
  ...
  file_stub <- file_stub_fun(cl)
  save_plot(p_forest, paste0("model_3_forest_", file_stub, "_mi"), 10, 6)
}
...
############################################################
########## 18. Forest plots for non-regression params ######
...
for (sex_i in group_levels) {
  ...
  plots_nu_forest[[sex_i]] <- make_nonreg_forest_plot(
    ...
    filename = paste0("model_3_forest_nu_", sex_i, "_mi"),
    width = 10,
    height = 7
  )
}
...
for (sex_i in group_levels) {
  ...
  plots_theta_forest[[sex_i]] <- make_nonreg_forest_plot(
    ...
    filename = paste0("model_3_forest_theta_", sex_i, "_mi"),
    width = 10,
    height = 8
  )
}
...
for (sex_i in group_levels) {
  ...
  plots_psi_var_forest[[sex_i]] <- make_nonreg_forest_plot(
    ...
    filename = paste0("model_3_forest_psi_variances_", sex_i, "_mi"),
    width = 10,
    height = 8
  )
}
...
for (sex_i in group_levels) {
  ...
  plots_psi_cov_forest[[sex_i]] <- make_nonreg_forest_plot(
    ...
    filename = paste0("model_3_forest_psi_covariances_", sex_i, "_mi"),
    width = 11,
    height = 10
  )
}
...
############################################################
###### 19. Density plots for non-regression params #########
###### memory-safe version: one family x sex at a time #####
############################################################

# Remove previous large intermediate object if it exists
...
for (i in seq_len(nrow(plot_specs_other))) {
  ...
  for (sex_i in group_levels) {
    ...
    # Small metadata table for this family and sex only
    ...
    # Filter the large draw table only for this family and sex.
    # Select minimal columns to reduce memory use.
    ...
    # Downsample early, before plotting.
    # This avoids keeping all posterior draws for all non-regression parameters.
    dat_i <- dat_i %>%
      sample_draws_for_plot() %>%
      mutate(
        group_label = sex_i,
    ...
    save_plot(
      p_i,
      paste0("model_3_density_", stub_i, "_", sex_i, "_mi"),
      width_i,
      height_i
    )
...
############################################################
########### 20. Robust vs uncertain effects summary ########
############################################################
...
print(p_robust_boys)
save_plot(p_robust_boys, "model_3_robustness_summary_boys_mi", 11, 7)
...
save_plot(p_robust_girls, "model_3_robustness_summary_girls_mi", 11, 7)

############################################################
############## 21. Heatmap of mean effects #################
############################################################
...
heatmap_data <- results_beta_plot %>%
  ...
  mutate(
    x_axis = factor(
      x_axis,
      levels = c("3y baseline", "5y residual", "3 -> 5", "5 -> 9", "9 -> 15", "15y outcome")
    )
  )
...
print(p_heatmap)
save_plot(p_heatmap, "model_3_heatmap_effects_mi", 14, 8)

############################################################
########## 22. Developmental variation in AR paths #########
############################################################
...
save_plot(p_trajectory_ar, "model_3_trajectory_ar_mi", 11, 6.5)

############################################################
######## 23. Forward cross-lagged developmental plot #######
############################################################
...
save_plot(p_trajectory_forward, "model_3_trajectory_forward_crosslag_mi", 11, 6.5)

############################################################
####### 24. Backward cross-lagged developmental plot #######
############################################################
...
save_plot(p_trajectory_backward, "model_3_trajectory_backward_crosslag_mi", 11, 6.5)

############################################################
######## 25. Lateral cross-lagged developmental plot #######
############################################################
...
save_plot(p_trajectory_lateral, "model_3_trajectory_ext_int_crosslag_mi", 11, 6.5)

############################################################
######### 26. Adversity effects at age 3 baseline ##########
############################################################
...
print(p_adv3)
save_plot(p_adv3, "model_3_adversity_3y_mi", 13, 7)

############################################################
######### 27. Adversity effects at age 5 residual ##########
############################################################
...
print(p_adv5)
save_plot(p_adv5, "model_3_adversity_5y_residual_mi", 13, 7)

############################################################
########### 28. Age-15 effects on mating outcome ###########
############################################################

trajectory_mating <- results_beta_plot %>%
  filter(class %in% c("adhd -> mating", "ext -> mating", "int -> mating")) %>%
  mutate(
    predictor = case_when(
      class == "adhd -> mating" ~ "ADHD",
      class == "ext -> mating" ~ "Externalizing",
      class == "int -> mating" ~ "Internalizing"
    )
  )

p_mating <- ggplot(trajectory_mating, aes(x = predictor, y = mean, colour = predictor)) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.3, colour = "grey50") +
  geom_point(size = 2.6) +
  geom_errorbar(aes(ymin = q2.5, ymax = q97.5), width = 0.15, linewidth = 0.5) +
  facet_wrap(~ group_label) +
  ...
  labs(
    title = "Age-15 psychopathology effects on mating behaviour",
    x = "Psychopathology dimension",
    y = "Posterior estimate",
    colour = "Predictor"
  ) +
  theme_results(12) +
...
print(p_mating)
save_plot(p_mating, "model_3_mating_15y_mi", 10, 6.5)

############################################################
######## 29. Age-15 effects on risky health outcome ########
############################################################

trajectory_health <- results_beta_plot %>%
  filter(class %in% c("adhd -> risky health", "ext -> risky health", "int -> risky health")) %>%
  mutate(
    predictor = case_when(
      class == "adhd -> risky health" ~ "ADHD",
      class == "ext -> risky health" ~ "Externalizing",
      class == "int -> risky health" ~ "Internalizing"
    )
  )

p_health <- ggplot(trajectory_health, aes(x = predictor, y = mean, colour = predictor)) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.3, colour = "grey50") +
  geom_point(size = 2.6) +
  geom_errorbar(aes(ymin = q2.5, ymax = q97.5), width = 0.15, linewidth = 0.5) +
  facet_wrap(~ group_label) +
  ...
  labs(
    title = "Age-15 psychopathology effects on risky health behaviour",
    x = "Psychopathology dimension",
    y = "Posterior estimate",
    colour = "Predictor"
  ) +
...
print(p_health)
save_plot(p_health, "model_3_risky_health_15y_mi", 10, 6.5)
...
print(list.files(getwd(), pattern = "^model_3_.*\\.(png|pdf)$"))

expected_files <- list.files(fig_dir, pattern = "^model_3_.*\\.(png|pdf)$", full.names = FALSE)
wd_files <- list.files(getwd(), pattern = "^model_3_.*\\.(png|pdf)$", full.names = FALSE)

cat("\nNumber of model_3 figure files in fig_dir:", length(expected_files), "\n")
cat("Number of model_3 figure files in working directory:", length(wd_files), "\n")
